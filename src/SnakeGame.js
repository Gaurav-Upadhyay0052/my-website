...
function SnakeGame() {
  ...
  const handleTouch = (dir) => {
    if (dir === "up" && moveRef.current.y !== 1)
      setDirection({ x: 0, y: -1 });
    if (dir === "down" && moveRef.current.y !== -1)
      setDirection({ x: 0, y: 1 });
    if (dir === "left" && moveRef.current.x !== 1)
      setDirection({ x: -1, y: 0 });
    if (dir === "right" && moveRef.current.x !== -1)
      setDirection({ x: 1, y: 0 });
  };
  ...
  return (
    <div className="snake-container">
      ...
      {/* On-screen touch controls (only visible on mobile/tablet) */}
